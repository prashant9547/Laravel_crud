...
@section('title', 'Unit | Shopping-Elite') 
@section('content')
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <table>
                    ...
                    <tr>
                        <td>{{ Form::label('unitName', ' unitName: ', array('class' => 'col-md-4 control-label')) }}</td>
                        <td>{{ Form::label('unitName', ' unitName ', array('class' => 'col-md-8','id'=> 'unitName')) }}</td>
                    </tr>
                    {{-- <tr>
...
@endsection
 @push("scripts") @if(Session::has('unit_update'))
<script>
    toastrDisplay("success","{{ Session::get('unit_update') }}");

</script>
@endif @if(Session::has('unit_create'))
<script>
    toastrDisplay("success","{{ Session::get('unit_create') }}");
...
<script>
    ...
    columns: [
        ...
        {data: null,  orderable: false, searchable: false,render:function(o){
        return '<a href="javascript:void(0);" data-toggle="modal" data-id="' + o.id + '" data-target="#mymodel" id="mymodel1"> <img src="{{ url("storage/images/view-icon.png")}}"> </a> <a href="unit/'+o.id+'/edit"> <img src="{{ url("storage/images/edit-icon.png")}}"> </a> <a href="javascript:void(0)" data-id="'+o.id+'" class="deleteadmin"> <img src="{{ url("storage/images/delete-icon.png")}}"> </i> </a>';
}}
    ]});
});
...
$(document).on('click','.deleteadmin',function(){
		var id=$(this).data("id");
		var delAdmin = confirm("Do you want to delete Unit ?");
            if (delAdmin == true) {
                $.ajax({
                    type: "DELETE",
                    url: "{{url('unit')}}/"+id,
